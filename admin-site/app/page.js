...
function Services({supabase,setNotice}){
 ...
 useEffect(()=>{load()},[]);
 async function add(e){e.preventDefault();const nextOrder=items.length?Math.max(...items.map(x=>Number(x.sort_order)||0))+10:10;const{error}=await supabase.from('services').insert({name,description,published:true,sort_order:nextOrder});if(error)setNotice(error.message);else{setName('');setDescription('');await load();setNotice('Repair service added.')}}
 ...
 return <section><div className="sectionHeading"><div><span className="kicker">REPAIR CONTENT</span><h1>Repair Services</h1><p>The order here is the order customers see on the Auto Repair page.</p></div></div>
