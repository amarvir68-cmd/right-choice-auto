...
export default async function Home(){
 ...
 return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:jsonLd(businessSchema)}}/><Header/><main>
  ...
  <section className="hero"><div><p className="eyebrow">WINNIPEG • AUTO REPAIR • USED CARS</p><h1>Quality Used Cars &amp;<br/><em>Expert Auto Repair.</em></h1><p>{settings.home_intro||'Quality used vehicles and dependable auto repair from a local Winnipeg shop.'}</p><div className="actions"><Link className="btn red" href="/cars">Browse Available Cars</Link><Link className="btn outline" href="/repairs#request-service">Request Repair Service</Link></div><p className="heroTrust">Winnipeg-based • Vehicle sales &amp; automotive repairs • Contact our team directly</p></div><div className="heroArt"><img className="heroLogo" src="/right-choice-logo.png" alt="Right Choice Auto Repair & Car Sales"/></div></section>
  ...
  <section className="section dark"><p className="eyebrow">AUTO REPAIR</p><h2>Reliable Auto Repair in Winnipeg</h2><div className="services">{[...new Map((services.length?services.map(s=>s.name):fallbackServices).map(n=>[n.trim().toLocaleLowerCase('en-CA'),n])).values()].slice(0,6).map(name=><div key={name}><span>◆</span><h3>{name}</h3></div>)}</div><div className="serviceActions"><Link className="btn red" href="/repairs#request-service">Request a Repair Appointment</Link><a className="btn outline" href={`tel:+1${(settings.phone||'204-632-4296').replace(/\D/g,'').replace(/^1/,'')}`}>Call: {settings.phone||'204-632-4296'}</a></div></section>
