...
export async function POST(request){
 try{
  ...
  const expectedOrigin=process.env.NEXT_PUBLIC_SITE_URL?.replace(/\/$/,'')||new URL(request.url).origin;
  if(origin&&origin!==expectedOrigin)return Response.json({error:'Invalid origin.'},{status:403});
  const fetchSite=request.headers.get('sec-fetch-site');
  if(fetchSite&& !['same-origin','none'].includes(fetchSite))return Response.json({error:'Cross-site submissions are not allowed.'},{status:403});
  if(!/^application\/json(?:;|$)/i.test(request.headers.get('content-type')||''))return Response.json({error:'JSON requests only.'},{status:415});
  ...
  if(captchaSecret){
   ...
   if(!result.ok)return Response.json({error:'Security verification unavailable. Try again later.'},{status:503});
   const verification=await result.json();if(!verification.success||verification.hostname!==new URL(expectedOrigin).hostname)return bad('Security check failed. Please try again.');
  }
