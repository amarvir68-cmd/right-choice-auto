...
export function VehicleActions({id,name}){const[saved,setSaved]=useState(false),[message,setMessage]=useState('');useEffect(()=>{try{setSaved(JSON.parse(localStorage.getItem('rc-favourites')||'[]').includes(id))}catch{}},[id]);function favorite(){try{let ids=JSON.parse(localStorage.getItem('rc-favourites')||'[]');ids=saved?ids.filter(x=>x!==id):[...new Set([...ids,id])];localStorage.setItem('rc-favourites',JSON.stringify(ids));setSaved(!saved)}catch{setMessage('Favourites are unavailable in this browser.')}}async function share(){if(navigator.share){try{await navigator.share({title:name,url:location.href});return}catch{return}}try{await navigator.clipboard.writeText(location.href);setMessage('Vehicle link copied.')}catch{setMessage('Copy this page URL from your address bar.')}}return <div className="vehicleActions"><button type="button" onClick={favorite} aria-pressed={saved}>{saved?'♥ Saved to favourites':'♡ Save vehicle'}</button><button type="button" onClick={share}>↗ Share vehicle</button>{message&&<small role="status">{message}</small>}</div>}
